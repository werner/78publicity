...
function cryout_proto_font($fonts,$sizes,$size,$font,$gfont,$labelsize,$labelfont,$labelgfont,$general=""){ ?>
	...
	<input class="googlefonts" type="text" size="35" value="<?php echo esc_attr($gfont); ?>"  name="tempera_settings[<?php echo $labelgfont; ?>]" id="<?php echo $labelgfont; ?>" placeholder = "<?php _e("or Google font","tempera"); ?>"/>
<?php
} // cryout_font_selector()


function cryout_color_field($id,$title,$value,$hint=""){
	echo '<input type="text" id="'.$id.'" class="colorthingy" title="'.$title.'" name="tempera_settings['.$id.']" value="'.esc_attr($value).'"  />';
