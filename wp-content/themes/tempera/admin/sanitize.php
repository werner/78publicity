...
/*** 2 ***/
	...
	for ($i=1;$i<10;$i+=2) {
		...
		if (in_array($input['tempera_social'.$i],$cryout_special_keys)) :
			...
			if ($input['tempera_social'.$i]=='Mail') {$input['tempera_social'.$j]='mailto:'.$input['tempera_social'.$j];};
			if ($input['tempera_social'.$i]=='Skype') {$input['tempera_social'.$j]='callto://'.$input['tempera_social'.$j];};
		else :
